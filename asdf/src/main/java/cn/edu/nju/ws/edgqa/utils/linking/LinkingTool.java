...
public class LinkingTool {
    ...
    private static final String falconServerlIP = "127.0.0.1";
    ...
    private static final String falconLiveURL = "https://labs.tib.eu/falcon/api?mode=long&k=3";

    private static final String earlURL = earlLocalURL;
    private static final String falconURL = falconLocalURL;

    /**
     * Enter a sentence and return the result Map of EARL Linking (including
     * relations and entities at the same time)
     *
    ...
    public static Map<String, List<Link>> getEARLLinking(String sentence) {
        System.out.println("EARLlinking");
        if (QAArgs.isUsingLinkingCache() && CacheUtil.getEarlMap() != null
                && CacheUtil.getEarlMap().containsKey(sentence))
    ...
    /**
     * Enter a sentence and return the result Map of falcon Linking (including
     * relations and entities at the same time). By default, each Mention returns 10
     * candidates
     *
     * @param sentence natural language question
    ...
    public static Map<String, List<Link>> getFalconLinking(String sentence, String linking_type) {
        ...
        while (tryTime < 2 && result.size() == 0) {
            ...
            int k;
            if (linking_type == "entity") {
                k = 80;
            } else {
                k = 10;
            }
            jsonObject.put("k", k);
            jsonObject.put("linking_type", linking_type);
            ...
            try {
                ...
                if (response != null) {
                    ...
                    // System.out.println(dbpediaEntities);
                    ...
                    // String ur123 = dbpediaEntities.getJSONArray(0).getString(0);
                    // System.out.println(ur123);
    ...
    public static Map<String, List<Link>> getDexterLinking(String sentence) {
        System.out.println("Dexterlinking");
        if (QAArgs.isUsingLinkingCache() && CacheUtil.getDexterMap() != null
                && CacheUtil.getDexterMap().containsKey(sentence))
    ...
    public static List<Link> getRelationLinkingByTool(String mention) {
        ...
        Map<String, List<Link>> falconLinking = LinkingTool.getFalconLinking(mention, "relation");
     ...
     */
    public static void getToolLinkingMaps(String sentence, Map<String, List<Link>> eLinkMap,
            Map<String, List<Link>> rLinkMap, ToolEnum tool) {
        Map<String, List<Link>> toolLinkingMap;
        Map<String, List<Link>> toolLinkingMap2 = new HashMap<>();
        if (tool == ToolEnum.EARL) {
            toolLinkingMap = getEARLLinking(sentence);
        } else if (tool == ToolEnum.FALCON) {
            toolLinkingMap = getFalconLinking(sentence, "entity");
            toolLinkingMap2 = getFalconLinking(sentence, "relation");
        ...
        // check null pointer
        if (eLinkMap == null)
            eLinkMap = new HashMap<>();
        if (rLinkMap == null)
            rLinkMap = new HashMap<>();
        ...
        // System.out.println("entity map: "+eLinkMap);

        for (String key : toolLinkingMap2.keySet()) {
            List<Link> links = toolLinkingMap2.get(key);

            if (!links.isEmpty()) { // not empty
                Map<String, List<Link>> targetMap2 = rLinkMap; // default rLinkMap
                if (targetMap2.containsKey(key)) {
                    targetMap2.get(key).addAll(links);
                } else {
                    targetMap2.put(key, new ArrayList<>(links));
                }
            }
        }
        // System.out.println("relation map: "+rLinkMap);
    }

    /**
     * Enter a sentence and return the link result after the fusion of multiple
     * tools
     *
     ...
     */
    public static void getEnsembleLinking(String sentence, Map<String, List<Link>> eLinkMap,
            Map<String, List<Link>> rLinkMap, boolean useFalcon) {
        ...
        // System.out.println("after rescore rLinkMap: "+ rLinkMap);
    }
    ...
    public static Map<String, List<Link>> spanConflictFix(String sentence, Map<String, List<Link>> eLinkMap) {
        ...
        if (spans.size() >= 2) {
            // The span to be deleted, removeFlag[i]=true indicates that the span at
            // position i should be deleted
                ...
                for (int j = i + 1; j < spans.size(); j++) {
                    ...
                    if (Span.spanConflict(span1, span2)) { // judge if two spans have overlap
                        ...
                        if (Math.abs(conf1 - conf2) < 0.0001) {// equal score
                            ...
                            if ((conf1 / conf2) < 1.25 && span2.covers(span1)) {// gap not very large, have another
                                                                                // better mention
                                if (conf1 >= 0.99) { // span1 perfect match
                                    if (conf2 >= 0.9 && span2.getLength() >= 30) { // span2 very long and similarity
                                                                                   // very high
                            ...
                            if ((conf2 / conf1) < 1.25 && span1.covers(span2)) {
                                if (conf2 >= 0.99) { // span2 perfect match
                                    if (conf1 >= 0.9 && span1.getLength() >= 30) { // span1 is very long and highly
                                                                                   // similar
                                        removeFlag[j] = true;
                                    } else {
                                        removeFlag[i] = true; // delete span1
    ...
    /**
     * The entity link results identified by the re-scoring tool will advance the
     * highest literal similarity and only return to top5
     *
    ...
    public static Map<String, List<Link>> reScoreEntity(Map<String, List<Link>> entityLinkMap) {
        ...
        for (Iterator<Map.Entry<String, List<Link>>> it = entityLinkMap.entrySet().iterator(); it.hasNext();) {
            ...
            if (linkList == null || linkList.isEmpty()) {// Check if it is empty
                ...
                if (linkList.get(0).getType() == LinkEnum.RELATION) {// It is a relation, skip it. This function only
                                                                     // reorders Entity
                    continue;
                }
            }
            ...
            while (iter.hasNext()) {
                ...
                // System.out.println("score before trimming the parentheses");
                double lexScore1 = SimilarityUtil.getScore(mention, label);// Scoring without removing the parentheses
                                                                           // of the disambiguation term
                // System.out.println('\n');

                // score after trimming the parentheses
                label = label.replaceAll("\\(.*", "").trim(); // Remove the brackets after the disambiguation item to
                                                              // avoid the disambiguation item is scored too low
                mention = mention.replaceAll("\\(.*", "").trim(); // Remove the disambiguation brackets in the mention
                // System.out.println("Score after removing parentheses");
                ...
                if (mention.toLowerCase().contains(label.toLowerCase().trim())) {// If mention contains label, increase
                                                                                 // the score appropriately
                    lexScore = Math.min(lexScore * 1.2, 1.0);
                }

                // For entities, it can be considered that literal similarity is the first
                // choice
            ...
            // double mentionConfidence = SimilarityUtil.getMentionConfidence(key,
            // entityLinkMap);
                ...
                if (rLinkList.get(0).getType() == LinkEnum.RELATION) { // Only deal with relation
                    ...
                    while (linkIterator.hasNext()) {
                        ...
                        if (uri.startsWith("http://dbpedia.org/ontology/")) {
                            ...
                            if (Character.isUpperCase(predicateName.charAt(0))) {// Ontology with the first letter
                                                                                 // capitalized, such as dbo:Writer,
                                                                                 // usually Class, does not constitute a
                                                                                 // relationship
    ...
    /**
     * Reorder the relational link list, put the same link dbo in advance and dbp in
     * the back
     *
    ...
    public static void reSortRelationLinkList(List<Link> relationLinkList) {
        ...
        while (iter.hasNext()) {
            ...
            if (uri.startsWith("http://dbpedia.org/ontology/")) {
                String[] strArr = uri.split("/");
                String predicateName = strArr[strArr.length - 1];
                if (Character.isUpperCase(predicateName.charAt(0))) {// Ontology with the first letter capitalized, such
                                                                     // as dbo:Writer, usually Class, does not
    ...
    public static boolean uriExists(String label, String uriType) {
        ...
        if (querySolutions.isEmpty())
            return false;
        return querySolutions.get(0).getLiteral("__ask_retval").toString()
                .equals("1^^http://www.w3.org/2001/XMLSchema#integer");
    }
    ...
    public static void literallyDetection(String sentence, HashMap<String, List<Link>> resultMap) {
        if (uriExists(sentence, "ontology")) {
            ...
            resultMap.get(sentence)
                    .add(new Link(sentence, "http://dbpedia.org/ontology/" + sentence, LinkEnum.ENTITY, 1.0));
        }
        if (uriExists(sentence, "resource")) {
            resultMap.computeIfAbsent(sentence, k -> new ArrayList<>());
            resultMap.get(sentence)
                    .add(new Link(sentence, "http://dbpedia.org/resource/" + sentence, LinkEnum.ENTITY, 1.0));
        }
    }

    /**
     * Recognize long entities in questions, based on dexter, earl and falcon
     * At the same time identify the names of people, places, institutions, etc. in
     * the question
     *
     * @param question  Question, and substitute the question
     * @param entityMap The mapping between the serial number after the substitution
     *                  and the label, such as <e0>:Obama
    ...
    public static String recognizeLongEntity(String question, Map<String, String> entityMap, KBEnum kb) {
        ...
        try {
            ...
            // Identify some noun phrases that are easily mis-segmented by the syntactic
            // tree
            ...
            if (kb == KBEnum.DBpedia) {
                ...
                for (String mention : dexterLinking.keySet()) {
                    if (!dexterLinking.get(mention).isEmpty()) {
                        ...
                        if (mentionLen < 4) {
                            dexterLinking.remove(mention);
                        }
                        if (mention.contains("fee")) {
                            dexterLinking.remove(mention);
                        } /*
                           * for(Link link : dexterLinking.get(mention)) {
                           * if(!link.getUri().contains("http")) {
                           * System.out.println("remove mention: "+mention);
                           * dexterLinking.remove(mention);
                           * //break;
                           * }
                           * if(link.getScore() < 0.04){
                           * System.out.println("remove mention: "+mention);
                           * dexterLinking.remove(mention);
                           * //break;
                           * }
                        ...
                        if (dexterLinking.get(mention).get(0).getType() == LinkEnum.ENTITY) { // Substitution entity
                            ...
                            if (mentionLen >= 3) {
                                if (!mention.trim().toLowerCase().startsWith("how many")
                                        && !mention.trim().toLowerCase().startsWith("give me")) { // how many nations
                                                                                                  // will be
                                                                                                  // misidentified
                                    // Identify disambiguation terms
                                    String regex = "(?i)" + mention.replace("+", "\\+").replace(".", "\\.")
                                            + "(\\s+\\(.*\\))";
                ...
                // earl link result
                // Map<String, List<Link>> earlLinking = getFalconLinking(question, "entity");
                // //System.out.println("earlLinking:" + earlLinking);
                // for (String mention : earlLinking.keySet()) {
                // //System.out.println("earlMention:"+mention);
                // if (!earlLinking.get(mention).isEmpty()) {
                // if (earlLinking.get(mention).get(0).getType() == LinkEnum.ENTITY) {
                // //Substitution entity only
                // int mentionLen = mention.split(" ").length;
                // if (mentionLen >= 3) {
                //
        ...
        if (jsonString != null && !jsonString.isEmpty()) {
            ...
            for (int i = 0; i < spots.length(); i++) {
                ...
                spots.getJSONObject(i).put("mention",
                        question.substring(spots.getJSONObject(i).getInt("start"),
                                spots.getJSONObject(i).getInt("end")));
    ...
    public static boolean isDexterEntity(String utterance, double e) {
        ...
        String jsonString = HttpsClientUtil.doPostWithParams(falconURL, inputObj);
        if (jsonString != null && !jsonString.isEmpty()) {
            JSONObject jsonObject = new JSONObject(jsonString);
    ...
    public static void main(String[] args) throws IOException {
        ...
        System.out.println("========================================");
        System.out.println("falcon linking:" + getFalconLinking(question, "enitiy"));
        System.out.println("========================================");
