...
public class KBUtil {
    ...
    private static final String CORE_KB_1604 = "http://localhost:8890";
    private static final String CORE_KB_GRAPH_1604 = "http://localhost:8890/CHT2";
...
//    private static final String CORE_KB_JDBC_1604 = "jdbc:http://dbpedia.org/sparql";
    private static final String CORE_KB_JDBC_1604 = "jdbc:virtuoso://localhost:1111/charset=UTF-8/log_enable=2";
    ...
    public static boolean isERexists(String eURI, String rURI) {
        String query = "ASK WHERE { {" + bracket(eURI) + " " + bracket(rURI) + " ?p} UNION {?p " + bracket(rURI) + " " + bracket(eURI) + "}.}";
    ...
    public static boolean isE1RE2exists(String eURI1, String rURI, String eURI2) {
        String query = "ASK WHERE { {" + bracket(eURI1) + " " + bracket(rURI) + " " + bracket(eURI2) + "} UNION {" + bracket(eURI2) + " " + bracket(rURI) + " " + bracket(eURI1) + "}.}";
        //System.out.println(query);
        return runAskOnVirtuoso(query);
    }
    ...
    public static boolean isThreeTupleListExists(List<ThreeTuple<String, String, Double>> tupleList) {
        ...
        for (ThreeTuple<String, String, Double> tuple : tupleList) {
            ...
            query = query + " {" + bracket(eURI) + " " + bracket(rURI) + " ?x} UNION {?x " + bracket(rURI) + " " + bracket(eURI) + "}.\n";
        }
        query += "}";
        return runAskOnVirtuoso(query);
    ...
    public static boolean isEPairInTwoHop(String eURI1, String eURI2) {
        ...
        String query = "ASK WHERE {{" + bracket(eURI1) + " ?p1 ?x" + "} UNION {?x ?p1 " + bracket(eURI1) + "}.\n"
                + "{" + bracket(eURI2) + " ?p2 ?x" + "} UNION {?x ?p2 " + bracket(eURI2) + "}.}\n";
    ...
    public static boolean isEPairInOneHop(String eURI1, String eURI2) {
        ...
        String query = "ASK WHERE { {" + bracket(eURI1) + " ?p " + bracket(eURI2) + "} UNION {" + bracket(eURI2) + " ?p " + bracket(eURI1) + "}.}";
        return runAskOnVirtuoso(query);
...
//        System.out.println(re);
    ...
    public static Set<String> oneHopProperty(String entityURI) {
        String sparql = ""
                + "select ?p where {\n"
                + "  {?x ?p " + bracket(entityURI) + "}\n"
                + "union\n"
                + "{" + bracket(entityURI) + " ?p ?y}"
                + "}";
...
//        System.out.println(ret);
    ...
    public static boolean isaTitle(String uri) {
        ...
        String sparqlString = "ASK WHERE{{?x dbp:title " + bracket(uri) + "} UNION {?x dbo:title " + bracket(uri) + "}.}";
    ...
    public static boolean isDisambiguationPage(String uri) {
        String sparqlString = "ASK WHERE{" + bracket(uri) + " dbo:wikiPageDisambiguates ?x}";
        return runAskOnVirtuoso(sparqlString);
    }
    ...
    public static boolean isRedirectPage(String uri) {
        String sparqlString = "ASK WHERE{" + bracket(uri) + " dbo:wikiPageRedirects ?x}";
        return runAskOnVirtuoso(sparqlString);
    }
    ...
    public static List<String> getTitleInstances(String uri) {
        String sparqlString = "SELECT ?x WHERE{{?x dbp:title " + bracket(uri) + "} UNION {?x dbo:title " + bracket(uri) + "}.}";
        //System.out.println(sparqlString);
        sparqlString = addPrefix(sparqlString);
    ...
    public static List<String> getRedirectPage(String uri) {
        String sparqlString = "SELECT ?x WHERE{" + bracket(uri) + " dbo:wikiPageRedirects ?x}";
        sparqlString = addPrefix(sparqlString);
        return getQueryStringResult(QueryFactory.create(sparqlString));
    }
    ...
    public static List<String> getRelBetweenEPair(String eURI1, String eURI2) {
        String query = "SELECT ?p WHERE{ { " + bracket(eURI1) + " ?p " + bracket(eURI2) + " } UNION {" + bracket(eURI2) + " ?p " + bracket(eURI1) + "}.}";
    ...
    public static Set<String> getSecondHopProperty(String mid, String firstHop) {
        ...
        String sparql = "select ?p where{ " + bracket(mid) + " " + bracket(firstHop) + " ?c . \n" +
                "?c ?p ?x .\n" +
                "filter(?x != " + bracket(mid) + ")}";
